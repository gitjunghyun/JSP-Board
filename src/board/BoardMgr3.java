...
public class BoardMgr3 {
	...
	private static BoardMgr3 pool = new BoardMgr3();

	// .jsp페이지에서 DB연동빈인 BoardMgr클래스의 메소드에 접근시 필요
	public static BoardMgr3 getInstance() {
		return pool;
	}
	...
	public Vector<BoardBean> getBoardList(String keyField, String keyWord, int start, int end) {
		...
		try {
			...
			if (keyWord.equals("null") || keyWord.equals("")) {
				sql = "select * from board_3 order by ref desc, pos limit ?, ?";
				// 모든 게시물을 가져오기 위한 SQL문
				pstmt = con.prepareStatement(sql);
				pstmt.setInt(1, start);
				pstmt.setInt(2, end);
			} else {
				sql = "select * from board_3 where " + keyField + " like ? ";
	...
	public int getTotalCount(String keyField, String keyWord) {
		...
		try {
			...
			if (keyWord.equals("null") || keyWord.equals("")) {
				sql = "select count(idx) from board_3";
				pstmt = con.prepareStatement(sql);
			} else {
				sql = "select count(idx) from board_3 where " + keyField + " like ? ";
				pstmt = con.prepareStatement(sql);
				pstmt.setString(1, "%" + keyWord + "%");
			}
			rs = pstmt.executeQuery();
	...
	public void insertBoard(HttpServletRequest req) {
		...
		try {
			con = pool.getConnection();
			sql = "select max(idx) from board_3";
			pstmt = con.prepareStatement(sql);
			rs = pstmt.executeQuery();
			int ref = 1;
			if (rs.next())
			...
			sql = "insert board_3(mem_id,content,title,ref,pos,depth,reg_date,pass,count,ip,filename,filesize)";
	...
	public BoardBean getBoard(int num) {
		...
		try {
			con = pool.getConnection();
			sql = "select * from board_3 where idx=?";
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, num);
			rs = pstmt.executeQuery();
	...
	public void upCount(int num) {
		...
		try {
			con = pool.getConnection();
			sql = "update board_3 set count=count+1 where idx=?";
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, num);
			pstmt.executeUpdate();
		} catch (Exception e) {
	...
	public void deleteBoard(int num) {
		...
		try {
			con = pool.getConnection();
			sql = "select filename from board_3 where idx=?";
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, num);
			rs = pstmt.executeQuery();
			...
			sql = "delete from board_3 where idx=?";
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, num);
			pstmt.executeUpdate();
		} catch (Exception e) {
	...
	public void updateBoard(BoardBean bean) {
		...
		try {
			con = pool.getConnection();
			sql = "update board_3 set mem_id=?,title=?,content=? where idx=?";
			pstmt = con.prepareStatement(sql);
	...
	public void replyBoard(BoardBean bean) {
		...
		try {
			con = pool.getConnection();
			sql = "insert board_3 (mem_id,content,title,ref,pos,depth,reg_date,pass,count,ip)";
			sql += "values(?,?,?,?,?,?,now(),?,0,?)";
	...
	public void replyUpBoard(int ref, int pos) {
		...
		try {
			con = pool.getConnection();
			sql = "update board_3 set pos = pos + 1 where ref=? and pos > ?";
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, ref);
			pstmt.setInt(2, pos);
			pstmt.executeUpdate();
